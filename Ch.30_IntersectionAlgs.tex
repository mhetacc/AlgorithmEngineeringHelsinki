...
\begin{itemize} 
    ...
    \item $b_j<min(i+2^k,n)$ i.e. current step or end of \textit{A}
\end{itemize}

We can therefore write $2^{k-1} \leq i-(i-1)$ where \textit{i} and \textit{(i-1)} are current and previous step respectively, and combining this inequality with $\Delta_i$ we get $\Delta_i \leq 2^{k-1} \leq i-(i-1)$. \\
...
For every \textit{i}, the algorithm executes $O(1+ \log \Delta_i)$ steps, thus summing for $i = 1,2,...,m$ (since $m=|B|$) we get a total time complexity of $\sum_{i=1}^m O(1+ \log \Delta_i)=O(\sum_{i=1}^{m}(1+log \Delta_i))=O(m+m \log \sum_{i=1}^m \frac{\Delta_i}{m})=O(m(1+ \log \frac{n}{m}))$.\\
This is the same time complexity we got for the \textit{divide and search} \brref{alg:divandsearch} algorithm, but this present algorithm paradigm is iterative, thus does not require dynamic memory allocation. Moreover, it calls the \textit{binary search} \brref{sec:binsearch} on a sub-array of \textit{A} needing less disk accesses.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \ForAll{$j=0$ to $m-1$}
            \State Let $k=0$
            \While{$B[j] > A[i+2^k]$ and $i+2^k \leq n$}
                \State $k=k+1$ \Comment Increment exponent
            \EndWhile
            \State $i'$ = \textit{binary search} into $A[i+2^{k-1}+1, min(i+2^k),n]$
            \If{$a_{i'}=b_j$}
