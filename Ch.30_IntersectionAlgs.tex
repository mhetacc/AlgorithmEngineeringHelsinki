...
Introduced by Barbay and Kenyon, it takes a collections of \textit{k} sorted sets ordered by size (ascending). Then it picks elements from the smallest one and searches for them in the other sets via \textit{galloping search} \brref{galloping}, one set at a time.\\
...
The pseudocode can be seen at \textit{Algorithm} \brref{alg:sequential}.

\begin{algorithm}
    \captionsetup{labelsep=newline}
    \caption{Pseudocode for sequential algorithm \label{alg:baezayates}}
    \begin{algorithmic}[1]
        \State Choose eliminator $e=set[0][0]$
        \State Let $i=0$
        \While{$e \neq \infty$}
    \end{algorithmic}
\end{algorithm}