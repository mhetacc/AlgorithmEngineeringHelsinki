...
\subsection{Sequential and Random Sequential}
...
\subsection{SvS and Swapping SvS}

\begin{figure}[H] 
    \begin{center}
        \includegraphics[width=.8\textwidth]{imgs/svs.png}
        \caption{SvS search algorithm \label{fig:svs}}
    \end{center}
\end{figure}
...
We first instantiate a value $\ell=0$ for all sets, which will be used to keep track of the last checked element in each set, acting as a sort of logical start position.\\
Then we pick elements from the smallest set $S_0$ and search for them in the second smallest set, updating $\ell$ as we go: if $key=10$ and $S_j[\alpha]=9$ then we won't need to ever check $S_j[0 \ldots \alpha]$. We can use any search algorithm we want. \\
Its variant pick the next search element from the new smallest set instead than picking all of them from the original $S_0$.

The pseudocode of the algorithm can be seen at \textit{Algorithm} \brref{alg:svds}

\begin{algorithm}
    \captionsetup{labelsep=newline}
    \caption{Pseudocode for doubling search algorithm \label{alg:svs}}
    \begin{algorithmic}[1]
        \State Sort sets by size \big($\big|set[0]\big| \leq \ldots \leq \big|set[k]\big|$\big)
    \end{algorithmic}
\end{algorithm}
